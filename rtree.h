...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool(const DATATYPE&)>)const;
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(const DATATYPE&)>)const;
  ...
  ObjectPool<ListNode>  m_nodeListPool;
public:
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool(const DATATYPE&)> a_resultCallback)const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
#else // RTREE_DONT_USE_MEMPOOLS
  // EXAMPLE
  m_nodeListPool.mFree(a_listNode);
#endif // RTREE_DONT_USE_MEMPOOLS
}
...
bool RTREE_QUAL::InsertRectRec(Rect* a_rect, const DATATYPE& a_id, Node* a_node, Node** a_newNode, int a_level)
{
  ...
  else if(a_node->m_level == a_level) // Have reached level for insertion. Add rect, split if necessary
  {
    ...
    branch.m_child = (Node*)(uintptr_t) a_id;
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
  {
    newRect.m_min[index] = Minimun(a_rectA->m_min[index], a_rectB->m_min[index]);
    newRect.m_max[index] = Maximum(a_rectA->m_max[index], a_rectB->m_max[index]);
  }
...
bool RTREE_QUAL::RemoveRectRec(Rect* a_rect, const DATATYPE& a_id, Node* a_node, ListNode** a_listNode)
{
  ...
  else // A leaf node
  {
    for(int index = 0; index < a_node->m_count; ++index)
    {
      if(a_node->m_branch[index].m_child == (Node*)(uintptr_t)a_id)
      {
        DisconnectBranch(a_node, index); // Must return after this call as count has changed
        return false;
      }
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool(const DATATYPE&)> a_resultCallback)const
{
  assert(a_node);
  assert(a_node->m_level >= 0);
  assert(a_rect);
